...
const authorize = (req) => {
  ...
  let tokenResponse = superagent.post('https://www.googleapis.com/oauth2/v4/token')
    .type('form')
    .send({
      code: code,
  ...
  let googleUserResponse = superagent.get('https://www.googleapis.com/plus/v1/people/me/openIdConnect')
  ...
  let mongoAccount = User.createFromOauth(googleUser);
    
  console.log('(5) Created User, generating token');
  let token = mongoAccount.generateToken();
